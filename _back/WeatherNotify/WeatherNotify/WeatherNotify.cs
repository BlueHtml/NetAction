...
#:package MailKit@4.9.0

using System.Text.Json;
using Util.Mail;

(string cityId, IEmailConfiguration emailConf, EmailAddress FromAddressObj, EmailAddress ToAddressObj) = Deserialize<Conf>(GetEnvValue("WeatherNotify_Conf".ToUpper()));
...
{
    var cityName = eleCityWeather.GetProperty("region").GetString();//这里要以实际情况为准，也有可能是 区，此时前面需要添加city。
...
record Conf(string cityId, EmailConfiguration emailConf, EmailAddress FromAddressObj, EmailAddress ToAddressObj);
